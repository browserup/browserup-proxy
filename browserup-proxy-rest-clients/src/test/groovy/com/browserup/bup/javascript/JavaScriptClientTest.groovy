...
package com.browserup.bup.javascript
...
class JavaScriptClientTest extends WithRunningProxyRestTest {
    ...
    void connectToProxy() {
        ...
        Assert.assertEquals("Expected javascript-client container exit code to be 0", 0, container.getCurrentContainerInfo().getState().getExitCode())
    }
    ...
    void failsToConnectToProxy() {
        ...
        Assert.assertEquals("Expected javascript-client container exit code to be 1", 1, container.getCurrentContainerInfo().getState().getExitCode())
    }
}
