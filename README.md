...
The latest version of BrowserMobProxy is 2.1.0-beta-2. It is the second release that supports the [new BrowserMobProxy interface](#new-browsermobproxy-api), and the second release [powered by LittleProxy](#littleproxy-support). We highly recommend that you use  2.1.0-beta-2 instead of the [previous 2.0.0 release](https://github.com/lightbody/browsermob-proxy/tree/2.0).
...
You'll find the standalone BrowserMob Proxy distributable zip at `browsermob-dist/target/browsermob-proxy-2.1.0-beta-3-SNAPSHOT-bin.zip`. Unzip the contents and run the `browsermob-proxy` or `browsermob-proxy.bat` files in the `bin` directory.
...
```xml
    <dependency>
        <groupId>net.lightbody.bmp</groupId>
        <artifactId>browsermob-core-littleproxy</artifactId>
        <version>2.1.0-beta-3-SNAPSHOT</version>
        <scope>test</scope>
    </dependency>
```